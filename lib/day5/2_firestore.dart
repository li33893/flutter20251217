...
class _MyAppState extends State<MyApp> {
  ...
  void _updateUser() async {
    ...
    QuerySnapshot snap = await users.where('name', isEqualTo: 'honggildong').get(); // 'hong' 사용자를 조회
    for (QueryDocumentSnapshot doc in snap.docs) {
      users.doc(doc.id).update({'age': 29}); // 나이 값을 30으로 업데이트
    }
  }
  ...
  void _deleteUser() async {
    ...
    QuerySnapshot snap = await users.where('name', isEqualTo: 'kimcheolsu').get(); // 'hong' 사용자를 조회
    for (QueryDocumentSnapshot doc in snap.docs) {
      users.doc(doc.id).delete(); // 나이 값을 30으로 업데이트
    }
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: _deleteUser, // 버튼 클릭 시 사용자 삭제 함수 호출
                  child: Text("사용자 삭제!"),
                ),
                SizedBox(height: 20),
                // 사용자 목록 표시
